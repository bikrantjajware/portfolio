  ...
  return (
    <div
      ...
      className={`relative rounded-lg overflow-hidden transition-transform transform hover:-translate-y-1 hover:shadow-lg ${
        isHovered
          ? 'bg-opacity-80 border-4 border-blue-400'
          : 'bg-opacity-100 border-4 border-gray-300'
      }`}>
      ...
      {isHovered && (
        <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-white bg-black bg-opacity-80">
          ...
          <p className="mt-2 text-sm text-sky-300">{description}</p>
          <a
            href={link}
            target="_blank"
            rel="noopener noreferrer"
            className="mt-4 hover:text-blue-400 text-blue-500 hover:underline">
            View Project
          </a>
        </div>
      )}
